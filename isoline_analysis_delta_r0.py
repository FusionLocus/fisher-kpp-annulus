...
if __name__ == '__main__':      
    ...
    plt.xticks(ticks= np.arange(0, r0_vals.size, step = 1), 
               labels=[fr'${r0:.1f}$' for r0 in r0_vals])
    #plt.ylim([0.1, 2.10])
    plt.ylabel(r'$\delta$')
    plt.yticks(ticks= np.arange(0, delta_vals.size, step = 1),
                labels=[fr'${delta:.1f}$' for delta in delta_vals])
    #plt.colorbar()
